...
import { searchSeries as searchDrama } from "@/lib/drama-api";
...
export default function SearchModal({ isOpen, onClose, onSelect }: { isOpen: boolean, onClose: () => void, onSelect?: (item: any) => void }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      setLoading(true);
      try {
        const [tmdbData, dramaData] = await Promise.all([
            fetchTMDB(endpoints.search, `query=${encodeURIComponent(query)}`),
            searchDrama(query)
        ]);

        const tmdbResults = filterSafeContent(tmdbData.results || []).filter((i: any) => i.poster_path || i.profile_path).slice(0, 8);
        
        const dramaResults = (dramaData || []).map((item: any) => ({
            ...item,
            id: `drama_${item.id}`,
            title: item.title_ar,
            name: item.title_ar,
            poster_path: item.poster,
            backdrop_path: item.banner,
            media_type: "tv",
            isDrama: true,
        ...
        setResults([...dramaResults, ...tmdbResults]);
      } catch (e) {
        console.error(e);
      } finally {
        setLoading(false);
                ...
                <div className="grid grid-cols-1 gap-3">
                  {results.map((item) => {
                    ...
                    const content = (
                      <div className="group flex items-center gap-6 rounded-3xl p-3 transition hover:bg-white/[0.05] border border-transparent hover:border-white/5 text-left w-full">
                        <div className={`relative flex-shrink-0 overflow-hidden rounded-2xl ${isPerson ? 'h-24 w-24' : 'h-24 w-40'}`}>
                             <Image 
                                src={getImageUrl(isPerson ? item.profile_path : (item.backdrop_path || item.poster_path), 'w185')} 
                                alt="" 
                                fill
                          ...
                          <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest">
                             {isPerson ? (
                                 ...
                                 <>
                                    ...
                                    <span className={`px-2 py-0.5 rounded text-[8px] ${item.isDrama ? 'bg-primary/20 text-primary border border-primary/20' : 'bg-white/10'}`}>
                                        {item.isDrama ? 'ARABIC_PREMIUM' : type}
                                    </span>
                                 </>
                             )}
                          </div>
                    ...
                    return (
                        <Link 
                           key={item.id} 
                           href={`${href}${item.isDrama ? `?title=${encodeURIComponent(item.title_ar)}` : ''}`} 
                           onClick={onClose}
                        >
                           {content}
                        </Link>
