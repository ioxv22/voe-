  ...
  return (
    <Link href={`/watch/${type}/${movie.id}`}>
      ...
      >
        ...
        {/* Saved Indicator */}
        {isSaved && (
          <div className="absolute top-2.5 left-2.5 z-10 px-2 py-0.5 rounded-md bg-[#E50914] text-[9px] font-bold text-white flex items-center gap-1">
        ...
        {rank && rank <= 10 && (
          <div className="absolute -left-2 bottom-3 z-20 flex items-center justify-center pointer-events-none">
            <span className="text-7xl font-black italic text-transparent select-none drop-shadow-[0_4px_10px_rgba(229,9,20,0.5)]" style={{ WebkitTextStroke: '2px #E50914' }}>
              {rank}
            </span>
          </div>
        )}
        ...
        <div className="absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-[#0B0B0B] via-[#0B0B0B]/60 to-transparent p-4 opacity-0 transition-all duration-300 group-hover:opacity-100 backdrop-blur-[2px]">
          ...
          <div className="mt-3 transform translate-y-3 transition-all duration-300 delay-75 group-hover:translate-y-0 text-right">
              ...
              <div className="flex items-center gap-2 text-[10px] text-[#B3B3B3] font-bold mt-1">
                  <span className="text-yellow-400">⭐ {movie.vote_average?.toFixed(1) || "7.8"}</span>
                  <span className="bg-[#333333] text-white px-1.5 py-0.2 rounded text-[8px] font-bold">4K</span>
              </div>
          </div>
        </div>
      </motion.div>
