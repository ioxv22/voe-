...
function BrowseContent() {
  ...
  useEffect(() => {
    async function load() {
        ...
        } else if (genreParam === "premium_arabic") {
            setIsDrama(true);
            setTitle("Premium Arabic Originals | حصريات فوز");
            const data = await fetchDramaSeries("most_viewed", 50);
            setMovies(data || []);
            setLoading(false);
            return;
        } else {
    ...
    <div className="pt-32 pb-20 px-4 lg:px-12">
        <div className="max-w-7xl mx-auto space-y-12">
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6">
                    {movies.map(m => (
                        isDrama ? <DramaCard key={m.id} series={m} /> : <MovieCard key={m.id} movie={m} />
                    ))}
