...
export const getImageUrl = (path: string, size: TMDBImageSize = "w500") => {
  if (!path) return "https://i.ibb.co/wrCgwgzt/Chat-GPT-Image-Apr-22-2026-09-29-48-PM.png";
  
  // Use weserv as a proxy to bypass potential region blocks and for additional caching/optimization
  // weserv requires a full URL with protocol to function correctly
  const originalUrl = `https://image.tmdb.org/t/p/${size}${path}`;
  
  // We encode the URL to ensure special characters don't break the proxy request
  return `https://images.weserv.nl/?url=${encodeURIComponent(originalUrl)}&default=https://i.ibb.co/wrCgwgzt/Chat-GPT-Image-Apr-22-2026-09-29-48-PM.png&output=webp&q=80`;
};
